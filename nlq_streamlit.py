...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# KNOWN VALUES FOR EXACT MATCHING
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

# "Material Type" (11 values - use exact match):
...
# "SOP Family" (21 values - use = not LIKE):
...
#   'MONO-CEL_D', 'CMPT', 'PKG', 'nVent Thermal Europe'
  
#   ⚠️  'SENSORS' means ONLY 'SENSORS' - does NOT include other SEN-* families
...
# 📌 PATTERN 2: TOP MATERIALS BY DEMAND (EXPLICIT REQUEST)
# Use ONLY when user specifically mentions "demand":
...
# 📌 PATTERN 3: FILTERING BY SOP FAMILY
# Always use exact match and NULL filter:
...
# 📌 PATTERN 4: AGGREGATION BY CATEGORY (SOP Family, Material Type, etc.)
...
# 📌 PATTERN 5: MATERIALS WITH FILTERS

# SELECT 
#     "Material Name",
#     "Material Type",
#     "UOM",
#     ROUND("Shelf Stock ($)", 2) AS "Shelf Stock Value ($)",
#     ROUND("Demand", 2) AS "Demand"
# FROM inventory
# WHERE "Material Type" = 'Raw materials'
#   AND "Material Name" IS NOT NULL
#   AND "Shelf Stock ($)" > 0
# ORDER BY "Shelf Stock ($)" DESC
# LIMIT 10;

# 📌 PATTERN 6: COMPARISON QUERIES (Demand vs Shelf Stock)
...
#         CASE 
#             WHEN SUM("Demand") > 0 
#             THEN (SUM("Shelf Stock ($)") / SUM("Demand")) * 100 
#             ELSE 0 
#         END, 
#         2
#     ) AS "Stock to Demand Ratio (%)"
# FROM inventory
# WHERE "Material Name" IS NOT NULL
# GROUP BY "Material Name"
# ORDER BY SUM("Shelf Stock ($)") DESC
...
# ❌ WRONG: Showing "Material" column
# ✅ RIGHT: Always show "Material Name"

# ❌ WRONG: Summing "Shelf Stock" across materials
# ✅ RIGHT: Sum "Shelf Stock ($)" for dollar values

# ❌ WRONG: Missing NULL filters
# ✅ RIGHT: Add IS NOT NULL for all categorical columns being filtered/grouped

# ❌ WRONG: Using LIKE for known SOP Family values
# ✅ RIGHT: Use exact match (=) for 'SENSORS', 'NUHEAT', etc.

# ❌ WRONG: Not rounding monetary values
# ✅ RIGHT: ROUND(SUM("Shelf Stock ($)"), 2)

# ❌ WRONG: Forgetting column quotes
# ✅ RIGHT: "Shelf Stock ($)" not Shelf Stock ($)

# ❌ WRONG: Sorting by demand when user just says "top materials"
# ✅ RIGHT: Default sort by "Shelf Stock ($)" DESC

# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# INTERPRETATION GUIDE
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

# User asks: "top materials" / "highest materials" / "materials with most shelf stock"
# → Sort by "Shelf Stock ($)" DESC (dollar value)

# User asks: "materials with highest demand" / "top demand"
# → Sort by "Demand" DESC

# User asks: "show SENSORS inventory" / "SENSORS shelf stock"
# → Filter WHERE "SOP Family" = 'SENSORS'

# User asks: "how many materials in NUHEAT"
# → COUNT(DISTINCT "Material Name") WHERE "SOP Family" = 'NUHEAT'

# User asks: "total value of inventory"
# → SUM("Shelf Stock ($)")

# User asks: "materials with no stock"
# → WHERE "Shelf Stock ($)" = 0
...
# 2. DEFAULT LIMIT: Use LIMIT 10 unless user specifies otherwise
# 3. CASE SENSITIVITY: SQLite is case-insensitive for column names but use exact case for values
# 4. NULL HANDLING: Always filter out NULLs explicitly with IS NOT NULL
# 5. PERFORMANCE: Use indexed columns in WHERE clauses when possible
...
# 1. **Brief acknowledgment** (1 sentence)
#    "I'll find the top 10 materials by shelf stock value."
...
# 4. **Summary** (1-2 sentences with key insights)
#    "The top material has $X in shelf stock, representing Y% of total inventory."
...
# □ All column names in double-quotes?
# □ NULL filters added for categorical columns?
# □ Using "Shelf Stock ($)" for dollar aggregations?
# □ Using "Material Name" not "Material"?
# □ ROUND() applied to monetary values?
# □ Correct sorting based on user's intent?
# □ Appropriate LIMIT set?

# Only execute if ALL boxes are checked.
# """


# # ---------------- AGENT WITH BETTER ERROR HANDLING ----------------
...
#     # Using gpt-4o with better parameters for accuracy
#     llm = ChatOpenAI(
#         model="gpt-4o",
#         temperature=0,  # Zero temperature for deterministic, accurate responses
#         api_key=api_key,
#         max_tokens=2000,  # Allow longer responses for complex queries
#     )
...
#         llm, 
#         tools, 
#         state_modifier=build_system_prompt()  # Using state_modifier for system instructions
#     )
